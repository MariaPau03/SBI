...
class MyModel(loopmodel):
    def select_loop_atoms(self):
        return selection(self.residue_range('246:A','275:A'))
    def special_restraints(self,aln):
        rsr = self.restraints
        at  = self.atoms
        rsr.add(secondary_structure.alpha(self.residue_range('250:A','273:A')))
        
a = MyModel(env,
              alnfile  = 'alignment5.pir', # alignment filename
